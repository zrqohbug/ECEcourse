...
class ProcAltCtrlPRTL( Model ):
  ...
  def __init__( s ):
    ...
    s.op2_sel_D       = OutPort( 3 )
    s.dmem_write_data_sel_D =\
                        OutPort( 2 )
    ...
    def comb_PC_sel_F():
      if s.pc_redirect_X:
        if s.inst_type_X == JALR:
          s.pc_sel_F.value = 3 # use JALR target 
        else:
    ...
    # select the correct output of X stage

    s.ex_result_sel_D = Wire( 2 )

    # bypass signals for D stage
    ...
    br_blt  = Bits( 3, 3 ) # branch less than
    br_bge  = Bits( 3, 4 ) # branch greater than or equal
    br_bltu = Bits( 3, 5 ) # branch less than unsigned
    br_bgeu = Bits( 3, 6 ) # branch greater than or equal unsigned

    # Op1 mux select

    am_x   = Bits( 3, 0 ) # don't care
    am_rf  = Bits( 3, 0 ) # use data from RF
    am_imm = Bits( 3, 1 ) # use imm
    am_bpX = Bits( 3, 2 ) # use bypass from X stage
    am_bpM = Bits( 3, 3 ) # use bypass from M stage
    am_bpW = Bits( 3, 4 ) # use bypass from M stage
    ...
    bm_bpX = Bits( 3, 3 ) # use bypass from X stage
    bm_bpM = Bits( 3, 4 ) # use bypass from M stage
    bm_bpW = Bits( 3, 5 ) # use bypass from W stage

    # dmem_write_data mux select

    dm_x   = Bits( 2, 0 ) # don't care
    dm_rf  = Bits( 2, 0 ) # use data from RF
    dm_bpX = Bits( 2, 1 ) # use bypass from X stage
    dm_bpM = Bits( 2, 2 ) # use bypass from M stage
    dm_bpW = Bits( 2, 3 ) # use bypass from W stage
    ...
    def comb_control_table_D():
      ...
      elif inst == AND    : cs( y, br_na,  imm_x, y, bm_rf,  y,  alu_and,    nr, wm_a, y,  n,   n    )
      elif inst == OR     : cs( y, br_na,  imm_x, y, bm_rf,  y,   alu_or,    nr, wm_a, y,  n,   n    )
      elif inst == XOR    : cs( y, br_na,  imm_x, y, bm_rf,  y,  alu_xor,    nr, wm_a, y,  n,   n    )
      elif inst == SLT    : cs( y, br_na,  imm_x, y, bm_rf,  y,  alu_slt,    nr, wm_a, y,  n,   n    )
      elif inst == SLTU   : cs( y, br_na,  imm_x, y, bm_rf,  y, alu_sltu,    nr, wm_a, y,  n,   n    )
      elif inst == SRA    : cs( y, br_na,  imm_x, y, bm_rf,  y,  alu_sra,    nr, wm_a, y,  n,   n    )
      elif inst == SRL    : cs( y, br_na,  imm_x, y, bm_rf,  y,  alu_srl,    nr, wm_a, y,  n,   n    )
      elif inst == SLL    : cs( y, br_na,  imm_x, y, bm_rf,  y,  alu_sll,    nr, wm_a, y,  n,   n    )
      elif inst == MUL    : cs( y, br_na,  imm_x, y, bm_rf,  y,  alu_mul,    nr, wm_a, y,  n,   n    )

      # Register-Immediate Arithmetic Instructions

      elif inst == ADDI   : cs( y, br_na,  imm_i, y, bm_imm, n,  alu_add,    nr, wm_a, y,  n,   n    )
      elif inst == ANDI   : cs( y, br_na,  imm_i, y, bm_imm, n,  alu_and,    nr, wm_a, y,  n,   n    )
      elif inst == ORI    : cs( y, br_na,  imm_i, y, bm_imm, n,   alu_or,    nr, wm_a, y,  n,   n    )
      elif inst == XORI   : cs( y, br_na,  imm_i, y, bm_imm, n,  alu_xor,    nr, wm_a, y,  n,   n    )
      elif inst == SLTI   : cs( y, br_na,  imm_i, y, bm_imm, n,  alu_slt,    nr, wm_a, y,  n,   n    )
      elif inst == SLTIU  : cs( y, br_na,  imm_i, y, bm_imm, n, alu_sltu,    nr, wm_a, y,  n,   n    )
      elif inst == SRAI   : cs( y, br_na,  imm_i, y, bm_imm, n,  alu_sra,    nr, wm_a, y,  n,   n    )
      elif inst == SRLI   : cs( y, br_na,  imm_i, y, bm_imm, n,  alu_srl,    nr, wm_a, y,  n,   n    )
      elif inst == SLLI   : cs( y, br_na,  imm_i, y, bm_imm, n,  alu_sll,    nr, wm_a, y,  n,   n    )
      elif inst == LUI    : cs( y, br_na,  imm_u, n, bm_imm, n,  alu_cp1,    nr, wm_a, y,  n,   n    )
      elif inst == AUIPC  : cs( y, br_na,  imm_u, n, bm_imm, n,  alu_add,    nr, wm_a, y,  n,   n    )

      # Memory Instructions

      elif inst == LW     : cs( y, br_na,  imm_i, y, bm_imm, n,  alu_add,    ld, wm_m, y,  n,   n    )
      elif inst == SW     : cs( y, br_na,  imm_s, y, bm_imm, y,  alu_add,    st, wm_x, n,  n,   n    )

      # Unconditional Jump Instructions
      elif inst == JAL    : cs( y, br_na,  imm_j, n, bm_x,   n,    alu_x,    nr, wm_a, y,  n,   n    )
      elif inst == JALR   : cs( y, br_na,  imm_i, y, bm_imm, n, alu_addz,    nr, wm_a, y,  n,   n    )

      # Conditional Branch Instructions

      elif inst == BNE    : cs( y, br_bne, imm_b, y, bm_rf,  y,    alu_x,    nr, wm_a, n,  n,   n    )
      elif inst == BEQ    : cs( y, br_beq, imm_b, y, bm_rf,  y,    alu_x,    nr, wm_a, n,  n,   n    )
      elif inst == BLT    : cs( y, br_blt, imm_b, y, bm_rf,  y,    alu_x,    nr, wm_a, n,  n,   n    )
      elif inst == BGE    : cs( y, br_bge, imm_b, y, bm_rf,  y,    alu_x,    nr, wm_a, n,  n,   n    )
      elif inst == BLTU   : cs( y, br_bltu,imm_b, y, bm_rf,  y,    alu_x,    nr, wm_a, n,  n,   n    )
      elif inst == BGEU   : cs( y, br_bgeu,imm_b, y, bm_rf,  y,    alu_x,    nr, wm_a, n,  n,   n    )
      ...
      # setting the bypass signals

      s.bypass_waddr_X_rs1_D.value =\
          s.val_D & s.rs1_en_D & s.val_X & s.rf_wen_pending_X &\
          ( s.inst_D[ RS1 ] == s.rf_waddr_X ) & ( s.rf_waddr_X != 0 ) &\
          ( s.inst_type_X != LW )

      s.bypass_waddr_X_rs2_D.value =\
          s.val_D & s.rs2_en_D & s.val_X & s.rf_wen_pending_X &\
          ( s.inst_D[ RS2 ] == s.rf_waddr_X ) & ( s.rf_waddr_X != 0 ) &\
          ( s.inst_type_X != LW )

      s.bypass_waddr_M_rs1_D.value =\
          s.val_D & s.rs1_en_D & s.val_M & s.rf_wen_pending_M &\
          ( s.inst_D[ RS1 ] == s.rf_waddr_M ) & ( s.rf_waddr_M != 0 )

      s.bypass_waddr_M_rs2_D.value =\
          s.val_D & s.rs2_en_D & s.val_M & s.rf_wen_pending_M &\
          ( s.inst_D[ RS2 ] == s.rf_waddr_M ) & ( s.rf_waddr_M != 0 )

      s.bypass_waddr_W_rs1_D.value =\
          s.val_D & s.rs1_en_D & s.val_W & s.rf_wen_pending_W &\
          ( s.inst_D[ RS1 ] == s.rf_waddr_W ) & ( s.rf_waddr_W != 0 )

      s.bypass_waddr_W_rs2_D.value =\
          s.val_D & s.rs2_en_D & s.val_W & s.rf_wen_pending_W &\
          ( s.inst_D[ RS2 ] == s.rf_waddr_W ) & ( s.rf_waddr_W != 0 )

      # setting up sel for Op1 mux

      if inst == AUIPC:
        s.op1_sel_D.value = am_imm
      elif s.bypass_waddr_X_rs1_D:
        s.op1_sel_D.value = am_bpX
      elif s.bypass_waddr_M_rs1_D:
        s.op1_sel_D.value = am_bpM
      elif s.bypass_waddr_W_rs1_D:
        s.op1_sel_D.value = am_bpW
      else:
        s.op1_sel_D.value = am_rf

      # setting up sel for Op2 mux
      # Note that s.op2_sel_D has already been set up in the control 
      # table so we only need to modify it when bypassing is needed

      if s.op2_sel_D == bm_rf:
        if s.bypass_waddr_X_rs2_D:
          s.op2_sel_D.value = bm_bpX
        elif s.bypass_waddr_M_rs2_D:
          s.op2_sel_D.value = bm_bpM
        elif s.bypass_waddr_W_rs2_D:
          s.op2_sel_D.value = bm_bpW

      # setting up sel for dmem_write_data mux

      if inst == SW:
        if s.bypass_waddr_X_rs2_D:
          s.dmem_write_data_sel_D.value = dm_bpX
        elif s.bypass_waddr_M_rs2_D:
          s.dmem_write_data_sel_D.value = dm_bpM
        elif s.bypass_waddr_W_rs2_D:
          s.dmem_write_data_sel_D.value = dm_bpW
        else:
          s.dmem_write_data_sel_D.value = dm_rf
      else:
        s.dmem_write_data_sel_D.value = dm_x

      # select the correct output of X stage

      if s.alu_fn_D == alu_mul:
        s.ex_result_sel_D.value = 0
      elif inst == JAL or inst == JALR:
        s.ex_result_sel_D.value = 2
      else:
        s.ex_result_sel_D.value = 1
    ...
    s.ostall_load_use_X_rs1_D = Wire( 1 )
    s.ostall_load_use_X_rs2_D = Wire( 1 )
    ...
    def comb_hazard_D():
      ...
      s.ostall_load_use_X_rs1_D.value =\
          s.val_D & s.rs1_en_D & s.val_X & s.rf_wen_pending_X &\
          ( s.inst_D[ RS1 ] == s.rf_waddr_X ) & ( s.rf_waddr_X != 0 ) &\
          ( s.inst_type_X == LW )

      s.ostall_load_use_X_rs2_D.value =\
          s.val_D & s.rs2_en_D & s.val_X & s.rf_wen_pending_X &\
          ( s.inst_D[ RS2 ] == s.rf_waddr_X ) & ( s.rf_waddr_X != 0 ) &\
          ( s.inst_type_X == LW )

      s.ostall_hazard_D.value =\
          s.ostall_load_use_X_rs1_D | s.ostall_load_use_X_rs2_D 

    # osquash due to control hazards

    s.osquash_j_D = Wire( 1 )

    @s.combinational
    def comb_squash_D():
      if s.inst_type_decoder_D.out == JAL:
        s.osquash_j_D.value = 1
      else:
        s.osquash_j_D.value = 0
    ...
    def jump_comb_D():
      ...
      if s.val_D and s.inst_type_decoder_D.out == JAL:
    ...
    def comb_D():
      ...
      s.ostall_D.value      = s.val_D & ( s.ostall_mngr_D |
          s.ostall_hazard_D | ~s.imul_req_rdy_D );
      ...
      s.osquash_D.value     = s.val_D & ( ~s.stall_D ) & s.osquash_j_D

      # squash in D stage

      s.squash_D.value      = s.val_D & s.osquash_X

      # imul_req_val_D needs to factor in stall_D

      s.imul_req_val_D.value = s.val_D & ( ~s.stall_D & ~s.squash_D ) &\
                                ( s.alu_fn_D == alu_mul )
    ...
    def comb_br_X():
      if s.val_X and ( s.br_type_X == br_bne ):
        ...
        s.pc_redirect_X.value = ~s.br_cond_ltu_X
      elif s.val_X and ( s.inst_type_X == JALR ):
    ...
    @s.combinational
    def comb_X():
      # ostall due to dmemreq and imul_resp

      s.ostall_X.value    = s.val_X & (\
                          ( ( s.dmemreq_type_X != nr ) & ~s.dmemreq_rdy ) \
                        | ( ( s.alu_fn_X == alu_mul ) & ~s.imul_resp_val_X ) \
                        )
      ...
      s.dmemreq_type.value    = 0 if s.dmemreq_type_X == ld else 1
      s.dmemreq_opaque.value  = Bits( 8, 0 )
      s.dmemreq_len.value     = Bits( 2, 0 )

      # setting imul_resp_rdy_X

      s.imul_resp_rdy_X.value = s.val_X & ~s.stall_X & \
                                ( s.alu_fn_X == alu_mul )
