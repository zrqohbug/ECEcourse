...
class ProcAltDpathPRTL( Model ):
  ...
  def __init__( s, num_cores = 1 ):
    ...
    # forward declaration for branch target and jal/jalr target
    ...
    # forward declarations for bypassing signals

    s.bypass_from_X   = Wire( 32 )
    s.bypass_from_M   = Wire( 32 )
    s.bypass_from_W   = Wire( 32 )

    # op1 sel mux
    # This mux chooses between RS1 and PC

    s.op1_sel_mux_D = m = Mux( dtype = 32, nports = 5 )
    s.connect_pairs(
      m.in_[0], s.rf_rdata0_D,
      m.in_[1], s.pc_reg_D.out,
      m.in_[2], s.bypass_from_X,
      m.in_[3], s.bypass_from_M,
      m.in_[4], s.bypass_from_W,
      m.sel,    s.op1_sel_D,
    )
    ...
    s.op2_sel_mux_D = m = Mux( dtype = 32, nports = 6 )
    s.connect_pairs(
      m.in_[0], s.rf_rdata1_D,
      m.in_[1], s.imm_gen_D.imm,
      m.in_[2], s.csrr_sel_mux_D.out,
      m.in_[3], s.bypass_from_X,
      m.in_[4], s.bypass_from_M,
      m.in_[5], s.bypass_from_W,
      m.sel,    s.op2_sel_D,
    )

    # dmem input selection mux
    # Bypassed values may be used as the input to dmem_req_msg

    s.dmem_write_data_mux_D = m = Mux( dtype = 32, nports = 4 )
    s.connect_pairs(
      m.in_[0], s.rf_rdata1_D,
      m.in_[1], s.bypass_from_X,
      m.in_[2], s.bypass_from_M,
      m.in_[3], s.bypass_from_W,
      m.sel,    s.dmem_write_data_sel_D,
    )
    ...
    s.imul_input = Wire( 64 )

    @s.combinational
    def imul_input_logic():
      s.imul_input.value = concat( s.op1_sel_mux_D.out, s.op2_sel_mux_D.out )

    s.imul_D = m = IntMulAltRTL()
    s.connect_pairs(
          m.req.val,  s.imul_req_val_D,
          m.req.rdy,  s.imul_req_rdy_D,
          m.req.msg,  s.imul_input, 
    ...
    s.connect_pairs(
      m.en,  s.reg_en_X,
      m.in_, s.dmem_write_data_mux_D.out,
    )
