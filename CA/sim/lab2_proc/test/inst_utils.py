...
def gen_st_template(
  ...
  return """
    ...
    csrr {reg_data}, mngr2proc < {result}
    {nops_data}
...
def gen_st_dest_dep_test( num_nops, inst, base, result ):
  return gen_st_template( 8, 8, num_nops, "x1", "x2", inst, 0, base, result )
...
def gen_st_base_dep_test( num_nops, inst, base, result ):
  return gen_st_template( num_nops, 0, 0, "x1", "x2", inst, 0, base, result )

#-------------------------------------------------------------------------
# gen_st_data_dep_test
#-------------------------------------------------------------------------
# Test the data register bypass paths by varying how many nops are
# inserted between writing the data register and reading this register in
# the instruction under test.

def gen_st_data_dep_test( num_nops, inst, base, result ):
  return gen_st_template( 0, num_nops, 0, "x1", "x2", inst, 0, base, result )
...
def gen_st_base_eq_dest_test( inst, base, result ):
  return gen_st_template( 0, 0, 0, "x1", "x3", inst, 0, base, result )

#-------------------------------------------------------------------------
# gen_st_data_eq_dest_test
#-------------------------------------------------------------------------
# Test situation where the data register specifier is the same as the
# destination register specifier.

def gen_st_data_eq_dest_test( inst, base, result ):
  return gen_st_template( 0, 0, 0, "x3", "x1", inst, 0, base, result )
...
def gen_st_value_test( inst, offset, base, result ):
  return gen_st_template( 0, 0, 0, "x1", "x2", inst, offset, base, result )

#-------------------------------------------------------------------------
# gen_jal_template
#-------------------------------------------------------------------------

def gen_jal_template( num_nops, num_blocks, seq, flow_pattern, addr_reg ):

  assert ( len( seq ) <= 8 ) and 1 <= num_blocks <= 8
  ...
  label_{label_num}:
    ...
    """.format(
        ...
        block_flag = b + 1,
        addr = 'final' if b == num_blocks-1 else seq[ 'addr' + str(b+2) ]
    )
    blocks.append( block )
...
{blocks}
  final:
    csrw  proc2mngr, x1 > {addr_reg}
    csrw  proc2mngr, x3 > {flow_pattern}
    """.format(
        nops = gen_nops( num_nops ), 
        blocks = '\n'.join( blocks ), 
...
def gen_jal_value_test( num_nops, num_blocks, seq, flow_pattern, addr_reg ):

  _seq = {}

  for idx, blk in enumerate( seq ):
    _idx = idx + 1
    _seq[ 'addr' + str(_idx) ] = 'label_' + str(blk)

  return gen_jal_template( num_nops, num_blocks, _seq, flow_pattern, addr_reg )

#-------------------------------------------------------------------------
# gen_jalr_template
#-------------------------------------------------------------------------

def gen_jalr_template( num_nops, num_blocks, seq, offset, flow_pattern, addr_reg ):
...
def gen_jalr_value_test( num_nops, num_blocks, seq, offset, flow_pattern, addr_reg ):
  ...
  return gen_jalr_template( num_nops, num_blocks, _seq, offset, flow_pattern, addr_reg )

#-------------------------------------------------------------------------
# gen_beq_jal_jalr_template
#-------------------------------------------------------------------------

def gen_beq_jal_jalr_template( num_blocks, seq, branch_jump, branch_reg ):
  ...
  for b in xrange( num_blocks ):
    block = """
  label_{label_num}:
    # Set up the flow pattern register x2
    slli  x2, x2, 4
    addi  x2, x2, {block_flag}
    # Set up the branch condition register x1
    addi  x1, x0, {branch_reg}
    # Set up the JALR register x3
    lui   x3,     %hi[{addr}]  
    addi  x3, x3, %lo[{addr}]
"""
    block = (block + '\n'.join( branch_jump[b] )).format(
        label_num = b + 1, 
        block_flag = b + 1,
        branch_reg = branch_reg[ b ],
        addr = 'final' if b == num_blocks-1 else seq[ 'addr' + str(b+2) ]
    )
    blocks.append( block )

  return """
    # Use x2 to track the control flow
  init:
    addi  x2, x0, 0
    lui   x3,     %hi[{addr1}]  
    addi  x3, x3, %lo[{addr1}]
...
{blocks}
  final:
    csrw  proc2mngr, x1 > 0x0
    csrw  proc2mngr, x2 > 0x0
    csrw  proc2mngr, x3 > 0x0
    csrw  proc2mngr, x4 > 0x0""".format(\
        blocks = '\n'.join( blocks ), 
        addr1 = seq[ 'addr1' ]
    )

#-------------------------------------------------------------------------
# gen_beq_jal_jalr_random_test
#-------------------------------------------------------------------------

def gen_beq_jal_jalr_random_test( num_blocks, seq ):

  assert num_blocks >= 1

  _seq = {}

  branch_reg = [ random.randint( 0, 1 ) for _ in xrange( num_blocks ) ]

  branch_jump = []

  for i in xrange( num_blocks ):
    insts = [ '    beq x1, x0, {addr}', '    jal x4, {addr}',
              '    jalr x4, x3, 0' 
            ]
    random.shuffle( insts )
    branch_jump.append( insts )

  for idx, blk in enumerate( seq ):
    _idx = idx + 1
    _seq[ 'addr' + str(_idx) ] = 'label_' + str(blk)

  return gen_beq_jal_jalr_template( num_blocks, _seq, branch_jump,\
      branch_reg )
...
def gen_mix_template( num_insts, num_blocks, num_regs, seq, ):
  ...
  block += '    jal   x0, {addr1}\n'.format( addr1 = seq[ 'addr1' ] )

  for b in xrange( num_blocks ):
    block += '  label_{label_num}:\n'.format( label_num = b+1 )

    for i in xrange( num_insts ):
      inst = random.randint( 1, 9 )
      if inst == 1:
        # mul
        block += '    mul   x{}, x{}, x{}\n'.format(\
        ...
        # sw
        addr = random.randint( 1, num_regs )
        block += '    andi x{}, x{}, 0x7ff\n'.format( addr, addr )
        block += '    sw   x{}, 0(x{})\n'.format(\
              random.randint( 1, num_regs ), 
              addr,
            )
  ...
  for i in xrange( num_regs ):
    block += '    csrw  proc2mngr, x{} > 0x0'.format( i+1 )
    if i != num_regs-1:
      block += '\n'
...
def gen_mix_random_test( num_insts, num_blocks, num_regs, seq ):
  ...
  return gen_mix_template( num_insts, num_blocks, num_regs, _seq )
