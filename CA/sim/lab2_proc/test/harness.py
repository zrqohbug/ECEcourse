...
from TestRefSink import TestRefSink
...
class TestHarness (Model):
  ...
  def __init__( s, ProcModel, dump_vcd,
                ...
                mem_stall_prob, mem_latency, ref_output = None, num_regs\
                =None):

    s.is_ref_model = True if ref_output != None else False

    s.src    = TestSource    ( 32, [], src_delay  )
    if s.is_ref_model:
      s.sink = TestRefSink   ( 32, ref_output, num_regs, sink_delay )
    else:
  ...
  def load( self, mem_image ):
    ...
    for section in sections:
      ...
      # Only do this for non-reference models

      elif section.name == ".proc2mngr":
        if not self.is_ref_model:
          for i in xrange(0,len(section.data),4):
            bits = struct.unpack_from("<I",buffer(section.data,i,4))[0]
...
def run_test( ProcModel, gen_test, dump_vcd=None,
              ...
              max_cycles=10000, print_trace = True, 
              ref_output = None, num_regs = 0 ):
